...
router.post("/register", async (req, res) => {
    ...
    try{
        const existingUser = await ChatUserss.findOne({ username });
        if(existingUser){
        ...
        const user = new ChatUserss({ username: username, password: hashPassword});
        await user.save();
...
router.post("/login", async (req, res) => {
    const { username, password } = req.body; 
    try {
        const user = await ChatUserss.findOne({ username });
